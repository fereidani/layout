...
fn index_vec_with_usize() {
    ...
    unsafe {
        assert_eq!(
            soa.get_unchecked_mut(0).name,
            &aos.get_unchecked_mut(0).name
        );
        assert_eq!(
            soa.get_unchecked_mut(0).mass,
            &aos.get_unchecked_mut(0).mass
        );
    }
...
fn index_vec_with_ranges() {
    ...
    let range = 0..1;
    eq_its(
        soa.get(range.clone()).unwrap().iter(),
        particles.get(range.clone()).unwrap().iter(),
    );
    unsafe {
        eq_its(
            soa.get_unchecked(range.clone()).iter(),
            particles.get_unchecked(range.clone()).iter(),
        );
    }
    eq_its(
        soa.index(range.clone()).iter(),
        particles[range.clone()].iter(),
    );
    eq_its(
        soa.get_mut(range.clone()).unwrap().iter(),
        particles.get(range.clone()).unwrap().iter(),
    );
    unsafe {
        eq_its(
            soa.get_unchecked_mut(range.clone()).iter(),
            particles.get_unchecked(range.clone()).iter(),
        );
    }
    ...
    let range = ..3;
    eq_its(
        soa.get(range).unwrap().iter(),
        particles.get(range).unwrap().iter(),
    );
    unsafe {
        eq_its(
            soa.get_unchecked(range).iter(),
            particles.get_unchecked(range).iter(),
        );
    }
    eq_its(soa.index(range).iter(), particles[range].iter());
    eq_its(
        soa.get_mut(range).unwrap().iter(),
        particles.get(range).unwrap().iter(),
    );
    unsafe {
        eq_its(
            soa.get_unchecked_mut(range).iter(),
            particles.get_unchecked(range).iter(),
        );
    }
...
fn index_slice_with_usize() {
    ...
    assert_eq!(
        soa_slice.get(0).unwrap().name,
        &aos_slice.first().unwrap().name
    );
    assert_eq!(
        soa_slice.get(0).unwrap().mass,
        &aos_slice.first().unwrap().mass
    );
    ...
    unsafe {
        assert_eq!(
            soa_slice.get_unchecked(0).name,
            &aos_slice.get_unchecked(0).name
        );
        assert_eq!(
            soa_slice.get_unchecked(0).mass,
            &aos_slice.get_unchecked(0).mass
        );
    }
    ...
    assert_eq!(
        soa_mut_slice.get_mut(0).unwrap().name,
        &aos_mut_slice.get_mut(0).unwrap().name
    );
    assert_eq!(
        soa_mut_slice.get_mut(0).unwrap().mass,
        &aos_mut_slice.get_mut(0).unwrap().mass
    );
    assert_eq!(
        soa_mut_slice.get_mut(0).unwrap().mass,
        &aos_mut_slice.get_mut(0).unwrap().mass
    ...
    unsafe {
        assert_eq!(
            soa_mut_slice.get_unchecked_mut(0).name,
            &aos_mut_slice.get_unchecked_mut(0).name
        );
        assert_eq!(
            soa_mut_slice.get_unchecked_mut(0).mass,
            &aos_mut_slice.get_unchecked_mut(0).mass
        );
    }
    ...
    *soa_mut_slice.index_mut(0).mass -= 1.;
    assert_eq!(
        soa_mut_slice.get(0).map(|p| *p.mass),
        Some(particle.mass - 1.)
    );
...
fn index_slice_with_ranges() {
    ...
    let range = 0..1;
    eq_its(
        soa_mut_slice.get(range.clone()).unwrap().iter(),
        particles.get(range.clone()).unwrap().iter(),
    );
    unsafe {
        eq_its(
            soa_mut_slice.get_unchecked(range.clone()).iter(),
            particles.get_unchecked(range.clone()).iter(),
        );
    }
    eq_its(
        soa_mut_slice.index(range.clone()).iter(),
        particles[range.clone()].iter(),
    );
    eq_its(
        soa_mut_slice.get_mut(range.clone()).unwrap().iter(),
        particles.get(range.clone()).unwrap().iter(),
    );
    unsafe {
        eq_its(
            soa_mut_slice.get_unchecked_mut(range.clone()).iter(),
            particles.get_unchecked(range.clone()).iter(),
        );
    }
    eq_its(
        soa_mut_slice.index_mut(range.clone()).iter(),
        particles[range].iter(),
    );

    let range = ..3;
    eq_its(
        soa_mut_slice.get(range).unwrap().iter(),
        particles.get(range).unwrap().iter(),
    );
    unsafe {
        eq_its(
            soa_mut_slice.get_unchecked(range).iter(),
            particles.get_unchecked(range).iter(),
        );
    }
    eq_its(soa_mut_slice.index(range).iter(), particles[range].iter());
    eq_its(
        soa_mut_slice.get_mut(range).unwrap().iter(),
        particles.get(range).unwrap().iter(),
    );
    unsafe {
        eq_its(
            soa_mut_slice.get_unchecked_mut(range).iter(),
            particles.get_unchecked(range).iter(),
        );
    }
    eq_its(
        soa_mut_slice.index_mut(range).iter(),
        particles[range].iter(),
    );
